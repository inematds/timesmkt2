...
- `<output_dir>/research_results.json` — trends, keywords, topics from Research Agent
- `<output_dir>/platforms/instagram.json` — Instagram copy (carousel, stories, reels, scheduling)
- `<output_dir>/platforms/youtube.json` — YouTube metadata (title, description, tags, scheduling)
- `<output_dir>/platforms/threads.json` — Threads posts (main, thread, standalone, scheduling)

Platform copy is produced by specialized platform agents in Stage 4. If platform files don't exist, log a warning in the Publish MD.
...
├── creative/
│   ├── creative_brief.json       ← Creative Director
│   └── creative_brief.md         ← Creative Director
├── copy/
│   ├── narrative.json            ← Copywriter (campaign narrative)
│   └── narrative.md              ← Copywriter (human-readable)
├── ads/                          ← Ad Creative Designer
│   ├── layout.json
│   ├── carousel_*.html / .png
│   └── story_*.html / .png
├── video/                        ← Video Quick or Video Pro
│   ├── video_*.mp4
│   └── video_*_scene_plan.json
├── platforms/                    ← Platform Agents (Stage 4)
│   ├── instagram.json / .md
│   ├── youtube.json / .md
│   └── threads.json / .md
